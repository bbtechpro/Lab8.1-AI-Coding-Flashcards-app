...
const modalSaveButton = modalForm.querySelector('button[type="submit"]');
...
function renderDecks() {
  decksList.textContent = '';

  if (!decks.length) {
    const emptyItem = document.createElement('li');
    emptyItem.className = 'empty';
    emptyItem.textContent = 'No decks yet. Create one to get started.';
    decksList.appendChild(emptyItem);
    return;
  }
  ...
  decks.forEach((deck) => {
    ...
    editButton.dataset.editDeck = deck.id;
    editButton.textContent = 'Edit';

    const deleteButton = document.createElement('button');
    deleteButton.type = 'button';
    deleteButton.className = 'delete-btn';
    deleteButton.dataset.deleteDeck = deck.id;
  ...
  decksList.appendChild(fragment);
}
...
function openModal(modalElement, opener) {
  ...
  window.requestAnimationFrame(() => {
    // Load card data after modal is shown to ensure inputs are updated
    if (modalElement === modal && cardModalMode === 'edit' && cardEditId) {
      const deck = decks.find((item) => item.id === selectedDeckId);
      if (deck) {
        const card = deck.cards.find((item) => item.id === cardEditId);
        if (card) {
...
function prepareForm(mode = 'create', type = 'deck', id = null) {
  ...
  // Reset all form inputs to prevent stale values
  deckNameInput.value = '';
  ...
  if (type === 'deck') {
    ...
    modalSaveButton.textContent = mode === 'edit' ? 'Update Deck' : 'Create Deck';
    const deck = decks.find((item) => item.id === id);
    if (deck) {
    ...
    modalSaveButton.textContent = mode === 'edit' ? 'Update Card' : 'Create Card';
  }
}
...
modalForm.addEventListener('submit', (event) => {
  ...
  // Example: Toggle 'required' based on visibility
// const input = document.querySelector('input[name="deckTitle"]');
